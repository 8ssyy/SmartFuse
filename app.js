...
window.navigate = function (viewId, el) {
    ...
    const titles = {
        ...
        smartia: 'Asistente IA SmartFuse'
    };
...
const API_KEY_GEMINI = 'AIzaSyDDYCquf9FyYrHm0juU-EzqF_AW-4iEitc';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btn) {
        ...
        console.error("ERROR CRÍTICO: No se encontró el botón con ID 'btn-consultar'");
    }
});
...
async function consultarIA() {
    ...
    // Debug checks
    if (!input) { console.error("Error: Input 'input-pregunta' no encontrado"); return; }
    if (!respuestaDiv) { console.error("Error: Contenedor 'respuesta-ia' no encontrado"); return; }

    const pregunta = input.value.trim();
    if (!pregunta) {
        console.warn("Input vacío, cancelando consulta.");
        return;
    }
    ...
    // 1. UI Updates
    agregarMensaje(respuestaDiv, 'user', pregunta);
    input.value = '';
    btn.disabled = true;
    ...
    try {
        ...
        agregarMensaje(respuestaDiv, 'bot', `Error: ${error.message}`);
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-paper-plane mr-1"></i> Consultar';
        console.log("Ciclo de consulta finalizado");
    }
}
...
function agregarMensaje(container, sender, text) {
    const div = document.createElement('div');
    const isUser = sender === 'user';

    div.className = isUser ? 'chat-msg-user animate-slideInRight' : 'chat-msg-bot animate-fadeInUp';
    ...
    // Auto-scroll
    const scrollParent = document.getElementById('chatContainer');
...
function construirContextoCompleto() {
    ...
    const pedidos = STORE.requests.map(r =>
        `- Pedido #${r.id}: ${r.item} (Prioridad: ${r.priority}) - Estado: ${r.status}`
    ).join('\n');

    const historial = STORE.history.slice(0, 5).map(h =>
...
[INVENTARIO DE FUSIBLES INSTALADOS]
${fusibles}
